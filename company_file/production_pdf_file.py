...
def read_data_from_database(cypher):
    ...
    #读取数据库的单词
    data_of_word_tolist = []
    data_of_word = graph.run("match (n:English_word) return distinct n.a_word,n.b_alphabet,n.c_meaning").to_data_frame()
    ...
    for i in range (len(data)):
        ...
        for i in range(len(word_different_from_text)):
            print(word_different_from_text[i])
...
def main():
    ...
    #开始读取数据库中文章的数据
    cypher = ("match (n:"+str(node_name)+") return n.a_reading,n.b_answer")
    ...
    #将生成的word文档存储在特定的位置
    file.save("D:\\python_word"+"\\"+str(file_name)+".docx")
    doc_name = "D:\\python_word"+"\\"+str(file_name)+".docx"
    ftp_name = "D:\\python_word\\" + str(file_name) + ".pdf"
